...
def adjust_learning_rate(optimizer, e, lr0=1e-6, warmup = 4, Tmax=epoch):
    if e < 3:
        lr = 100*lr0*10**(-e)
    elif e < warmup:
        lr = lr0
    ...
    for e in range(epoch):
        ...
        for phase in ['train','val']:
            ...
            for i,(x,t) in enumerate(data_loader[phase]):
                ...
                if i%10==0: 
                    print(f'{i}\t{(time()-t0)/(i+1):1.2f}s\t{sum_loss/sum_tot:1.4e}\t{recall:1.4f}\t{prec:1.4f}\t{y.mean().item():1.4f}')
